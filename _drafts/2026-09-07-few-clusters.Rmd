...
With six clusters vanilla cluster SEs cover the truth only `r cv("Clustered CR1, t(n-k)", 6)` of the time; regular standard errors are hopeless at `r cv("Regular OLS", 6)`. The wild cluster bootstrap gets to `r cv("Wild cluster bootstrap", 6)`. Its intervals are somewhat wider and more variable in width.
...
2. **Vanilla cluster SEs undercover below 30 or so clusters, and badly below 10.** At $G = 10$ it covers `r cv("Clustered CR1, t(n-k)", 10)`; at $G = 5$, `r cv("Clustered CR1, t(n-k)", 5)`.
