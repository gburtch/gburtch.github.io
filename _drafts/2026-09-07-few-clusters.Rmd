...
2. **Vanilla cluster SEs undercover below 30 or so clusters, and badly below 10.** At $G = 10$ it covers `r cv("CR1, t(n-k): R sandwich + coeftest default", 10)`; at $G = 5$, `r cv("CR1, t(n-k): R sandwich + coeftest default", 5)`.
3. **Using $t(G-1)$ critical values is free and helps.** Same variance estimate, coverage of `r cv("CR1, t(G-1): Stata / R fixest default", 10)` at $G = 10$ and `r cv("CR1, t(G-1): Stata / R fixest default", 5)` at $G = 5$. If your software does this by default (Stata and `fixest` do), you are already better off than the 2021 thread was.
