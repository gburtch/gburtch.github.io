...
2.  **CR1, $t(n-k)$.** The usual cluster-robust sandwich with Stata’s $\frac{G}{G-1}\frac{n-1}{n-k}$ finite-sample factor, paired with the same critical value OLS would use. This is what the 2021 thread did.
3.  **CR1, $t(G-1)$.** Identical variance estimate, but with critical values from a $t$ distribution with $G-1$ degrees of freedom. This is the default in Stata’s `vce(cluster)` and in R’s `fixest`, and it is the cheapest fix available: it acknowledges that the effective sample size for the variance is the number of clusters.
4.  **CR2 + Satterthwaite.** The bias-reduced CR2 sandwich of Bell and McCaffrey (2002), with Satterthwaite degrees of freedom, via `clubSandwich`. This is the correction Alexander added.
...
I run 1000 replications for every cluster count from 2 to 50. That is roughly 49,000 regressions, each with a bootstrap attached, so the work is spread across cores with `future.apply`. On a 16-core laptop it takes about five minutes.
